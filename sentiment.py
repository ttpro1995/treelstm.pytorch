...
def main():
    ...
    args.input_dim,  args.word_dim, args.tag_dim, args.rel_dim = 300, 300, 50, 50
    print (args.name)
    ...
    if os.path.isfile(train_file):
        ...
        train_dataset = SSTDataset(train_dir, vocab, tagvocab, relvocab, args.num_classes, args.fine_grain, args.model_name)
    ...
    if os.path.isfile(dev_file):
        ...
        dev_dataset = SSTDataset(dev_dir, vocab, tagvocab, relvocab, args.num_classes, args.fine_grain, args.model_name)
    ...
    if os.path.isfile(test_file):
        ...
        test_dataset = SSTDataset(test_dir, vocab, tagvocab, relvocab, args.num_classes, args.fine_grain, args.model_name)
    ...
    model = TreeLSTMSentiment(
                ...
                args.num_classes, criterion, args.model_name
            )
    ...
    mode = 'EXPERIMENT'
    dev_loss, dev_pred = trainer.test(dev_dataset)  # make sure thing go smooth before train
    if mode == 'DEBUG':
        ...
        for epoch in range(args.epochs):
            ...
            if epoch%5 == 0:# save at least 1 hours
                ...
                train_acc = metrics.sentiment_accuracy_score(train_pred, train_dataset.labels, args.fine_grain)
                print('Train acc %f ' % (train_acc))
            dev_loss, dev_pred = trainer.test(dev_dataset)
            dev_acc = metrics.sentiment_accuracy_score(dev_pred, dev_dataset.labels, args.fine_grain)
        ...
        test_loss, test_pred = trainer.test(test_dataset)
        test_acc = metrics.sentiment_accuracy_score(test_pred, test_dataset.labels,  args.fine_grain)
